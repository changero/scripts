  ...
  for (var a = 0; a < pan_title.length; a++) {
    if (pan_link.indexOf(pan_title[a]) != -1) {
      ...
      var pan_html =
        '<a href=' +
        pan_link +
        " target='_blank' style='cursor:pointer;z-index:98;display:block;width:30px;height:30px;line-height:30px;position:fixed;left:0;top:300px;text-align:center;'><img src='https://www.eggvod.cn/images/vip.png' style='width:50px' ></a>";
      ...
      (function () {
        ...
        let util = {
            ...
            try {
              GM_cookie &&
                GM_cookie('list', { name: 'BDUSS' }, (cookies, error) => {
                  if (!error) {
                    this.setStorage(
                      'baiduyunPlugin_BDUSS',
                      JSON.stringify({ BDUSS: cookies[0].value })
                    );
                  }
                });
            } catch (e) {}
          ...
          getBDUSS() {
            let baiduyunPlugin_BDUSS = this.getStorage('baiduyunPlugin_BDUSS')
              ? this.getStorage('baiduyunPlugin_BDUSS')
              : '{"baiduyunPlugin_BDUSS":""}';
          ...
          convertLinkToAria(link, filename, ua) {
            ...
            if (!!BDUSS) {
              return encodeURIComponent(
                `aria2c "${link}" --out "${filename}" --header "User-Agent: ${ua}" --header "Cookie: BDUSS=${BDUSS}"`
              );
            }
            return {
              link: pan.assistant,
          ...
          convertLinkToCurl(link, filename, ua) {
            ...
            if (!!BDUSS) {
              return encodeURIComponent(
                `curl -L "${link}" --output "${filename}" -A "${ua}" -b "BDUSS=${BDUSS}"`
              );
            }
            return {
              link: pan.assistant,
            ...
            return new Promise((resolve, reject) => {
              let requestObj = GM_xmlhttpRequest({
                ...
                onload: (res) => {
                  ...
                  if (type === 'blob') {
                    res.status === 200 &&
                      util.blobDownload(res.response, extra.filename);
                    resolve(res);
                  } else {
                    resolve(res.response || res.responseText);
                  }
        ...
        let panlinker = {
          initValue() {
            ...
            value.forEach((v) => {
              util.getValue(v.name) === undefined &&
          ...
          addPageListener() {
              ...
              ins[index] = setInterval(() => {
                ...
                if (isIDM) {
                  ...
                  if (prog == 100) {
                    ...
                    o.item
                      .find('.pl-progress-tip')
                      .html('下载完成，正在弹出浏览器下载框！');
                  }
                }
              }, 500);
            });
            ...
            doc.on('click', '.listener-link-aria, .listener-copy-aria', (e) => {
              ...
              if (!e.target.dataset.link) {
                $(e.target)
                  .removeClass('listener-copy-aria')
                  .addClass('pl-btn-danger')
                  .html(
                    `${pan.init[5]}👉<a href="${pan.assistant}" target="_blank">点击此处安装</a>👈`
                  );
              } else {
            ...
            doc.on('click', '.listener-link-rpc', async (e) => {
              ...
              let res = await this.sendLinkToRPC(
            ...
            doc.on('click', '.listener-rpc-task', () => {
              let rpc = JSON.stringify({
                ...
                url = `http://d.baiduyun.wiki/?rpc=${util.e(
                  rpc
                )}#${util.getValue('setting_rpc_token')}`;
              ...
              if (e.type === 'mouseenter') {
                ...
                $('.pl-tooltip')
                  ...
                  .css({
                    left: e.pageX + 10 + 'px',
                    top:
                      e.pageY - e.currentTarget.offsetTop > 14
                        ? e.pageY + 'px'
                        : e.pageY + 20 + 'px',
                  })
                  .show();
          ...
          addButton() {
            if (!pt) return;
            if (
              ($('.pl-button').length || $('.pl-button-init').length) &&
              pan.name !== name
            )
              return;
            ...
            let $toolWrap;
            let $button = $(
              `<span class="g-dropdown-button pointer pl-button"><a style="color:#fff;background: ${color};border-color:${color}" class="g-button g-button-blue" href="javascript:;"><span class="g-button-right"><em class="icon icon-download"></em><span class="text" style="width: 60px;">下载助手</span></span></a><span class="menu" style="width:auto;z-index:41;border-color:${color}"><a style="color:${color}" class="g-button-menu pl-button-mode" data-mode="api" href="javascript:;">API下载</a><a style="color:${color}" class="g-button-menu pl-button-mode" data-mode="aria" href="javascript:;" >Aria下载</a><a style="color:${color}" class="g-button-menu pl-button-mode" data-mode="rpc" href="javascript:;">RPC下载</a><a style="color:${color}" class="g-button-menu pl-button-mode" data-mode="curl" href="javascript:;">cURL下载</a>${
                pan.code == 200 && version < pan.version ? pan.new : ''
              }</span></span>`
            );
          ...
          addInitButton() {
            ...
            let $button = $(
              `<span class="g-dropdown-button pointer pl-button-init" style="opacity:.5"><a style="color:#fff;background: ${color};border-color:${color}" class="g-button g-button-blue" href="javascript:;"><span class="g-button-right"><em class="icon icon-download"></em><span class="text" style="width: 60px;">下载助手</span></span></span></a></span>`
            );
            if (pt === 'home') $toolWrap = $(pan.btn.home);
            console.log(pt);
            if (pt === 'main') {
              $toolWrap = $(pan.btn.main);
              $button = $(
                `<a href="javascript:;" class="pl-button-init" style="opacity:.5; display: inline-block; margin-right: 8px;"><button class="u-btn nd-file-list-toolbar-action-item u-btn--primary u-btn--default u-btn--small is-round is-has-icon" style="background: ${color};border-color: ${color}"><i class="iconfont inline-block-v-middle nd-file-list-toolbar__action-item-icon icon-download"></i><span class="inline-block-v-middle nd-file-list-toolbar-action-item-text">下载助手</span></button></a>`
              );
            }
            if (pt === 'share') $toolWrap = $(pan.btn.share);
            $toolWrap.prepend($button);
          ...
          generateDom(list) {
            ...
            list.forEach((v, i) => {
              ...
              if (mode === 'aria') {
                ...
                if (typeof alink === 'object') {
                  content += `<div class="pl-item">
                                <div class="pl-item-name listener-tip" data-size="${size}">${filename}</div>
                                <a class="pl-item-link" target="_blank" href="${
                                  alink.link
                                }" title="点击复制aria2c链接" data-filename="${filename}" data-link="${
                    alink.link
                  }">${decodeURIComponent(alink.text)}</a> </div>`;
                } else {
                  alinkAllText += alink + '\r\n';
                  content += `<div class="pl-item">
                                <div class="pl-item-name listener-tip" data-size="${size}">${filename}</div>
                                <a class="pl-item-link listener-link-aria" href="${alink}" title="点击复制aria2c链接" data-filename="${filename}" data-link="${alink}">${decodeURIComponent(
                    alink
                  )}</a> </div>`;
                }
              }
              ...
              if (mode === 'curl') {
                ...
                if (typeof alink === 'object') {
                  content += `<div class="pl-item">
                                ...
                                <a class="pl-item-link" target="_blank" href="${
                                  alink.link
                                }" title="点击复制curl链接" data-filename="${filename}" data-link="${
                    alink.link
                  }">${decodeURIComponent(alink.text)}</a> </div>`;
                } else {
                  alinkAllText += alink + '\r\n';
                  content += `<div class="pl-item">
                                <div class="pl-item-name listener-tip" data-size="${size}">${filename}</div>
                                <a class="pl-item-link listener-link-aria" href="${alink}" title="点击复制curl链接" data-filename="${filename}" data-link="${alink}">${decodeURIComponent(
                    alink
                  )}</a> </div>`;
                }
              }
            ...
            if (mode === 'rpc')
              content +=
                '<div class="pl-extra"><button class="pl-btn-primary  listener-send-rpc">发送全部链接</button><button class="pl-btn-primary listener-config-rpc" style="margin-left: 10px;">配置RPC服务</button><button class="pl-btn-primary pl-btn-success listener-rpc-task" style="margin-left: 10px;display: none">查看下载任务</button></div>';
            return content;
          },
          ...
          selectedList() {
            try {
              ...
              return document.querySelector('.nd-main-layout').__vue__
                .$children[2].selectedList;
            }
          },

          getLogid() {
            let ut = require('system-core:context/context.js').instanceForSystem
              .tools.baseService;
          ...
          async initPanLinker() {
            pt = this._detectPage();
            let res = await util.post(
              `https://api.baiduyun.wiki/config?ver=${version}&a=${author}`,
              {},
              {},
              'text'
            );
            ...
            pan.num === util.getValue('setting_init_code') ||
            pan.num === util.getValue('scode')
              ? this.addButton()
              : this.addInitButton();
          },
          ...
          showSetting() {
            ...
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC主机</div><input type="text"  placeholder="主机地址，需带上http(s)://" class="pl-input listener-domain" value="${util.getValue(
              'setting_rpc_domain'
            )}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC端口</div><input type="text" placeholder="端口号，例如：Motrix为16800" class="pl-input listener-port" value="${util.getValue(
              'setting_rpc_port'
            )}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">RPC密钥</div><input type="text" placeholder="无密钥无需填写" class="pl-input listener-token" value="${util.getValue(
              'setting_rpc_token'
            )}"></label>`;
            dom += `<label class="pl-setting-label"><div class="pl-label">保存路径</div><input type="text" placeholder="文件下载后保存路径，例如：D:" class="pl-input listener-dir" value="${util.getValue(
              'setting_rpc_dir'
            )}"></label>`;

            colorList.forEach((v) => {
              btn += `<div data-color="${v}" style="background: ${v};border: 1px solid ${v}" class="pl-color-box listener-color ${
                v == util.getValue('setting_theme_color') ? 'checked' : ''
              }"></div>`;
            });
...
(function () {
  ...
  if (couponUrl.indexOf('taobao') != -1 || couponUrl.indexOf('tmall') != -1) {
    $.get(
      'https://www.zuihuimai.net/vrhr/loading.php',
    ...
    $.get(
      ...
      function (loading_html) {
        if (loading_html) {
          ...
          $.get(
            'https://zuihuimai.net/vrhr/index_jd.php',
            { goods_id: jdGoodsId[1] },
            function (data) {
              if (data != '') {
                var jd_html = data;
  ...
  if (get_url.indexOf('eggvod.cn') == -1) {
    ...
    for (var n = 0; n < jx_title.length; n++) {
      if (get_url.indexOf(jx_title[n]) != -1) {
        ...
        var nnn = 0;
        for (
          var playLineIndexMobile = 0;
          playLineIndexMobile < play_line_json.length;
          playLineIndexMobile++
        ) {
        ...
        for (
          ...
          if (/(^[1-9]\d*$)/.test(playLineIndex / 3)) {
            ...
            play_wrap_html +=
              "<td class='playLineTd' id='" +
              play_line_json[playLineIndex].url +
              "' style='width:33%;color:#fff;text-align:center;background-color:#f24443;box-shadow:0px 0px 10px #fff;font-size:14px;'>" +
              play_line_json[playLineIndex]['name'] +
              '</td>';
        ...
        setTimeout(function () {
          ...
          if (play_url.indexOf('iqiyi.com/v_') != -1) {
            function iqiyiRemove() {
              ...
              removeAll(
                'div[style*="visibility"][style*="visible"]:not([class]):not([id]):not([style*="fixed"])',
                undefined,
                false
              );
            }
            ...
            (function () {
              $('body').on(
                ...
                function (e) {
                  let objfj = $(this),
                    href = objfj.attr('href') || objfj.data('href');
          ...
          $('.playLineTd').click(function () {
            ...
            var play_html =
              "<div style='width:100%;height:100%;'><iframe allowtransparency=true frameborder='0' scrolling='no' allowfullscreen=true allowtransparency=true name='jx_play' style='height:100%;width:100%' id='playIframe'></iframe></div>";
          ...
          $('#' + id_random).click(function () {
            ...
            if (
              ...
              mobile_html +=
                "<button type='button' style='position:absolute;top:0;right:30px;font-size:30px;line-height: 1;color: #000;text-shadow: 0 1px 0 #fff;cursor: pointer;border:0;background:0 0;' onclick='location.reload();'>×</button>";
              mobile_html +=
                "<div><iframe src='https://www.eggvod.cn/mobile.php?zhm_jx=" +
                play_jx_url +
                "' allowtransparency=true frameborder='0' scrolling='no' allowfullscreen=true allowtransparency=true name='jx_play'style='height:600px;width:100%'></iframe></div>";
              mobile_html += '</div>';
              $('body').html(mobile_html);
            } else {
              $.get(
                'https://www.eggvod.cn/jxcode.php',
                { in: 81516699, code: 4 },
                function (data) {
                  location.href =
                    'https://www.eggvod.cn/jxjx.php?lrspm=' +
                    data +
                    '&zhm_jx=' +
                    play_jx_url;
                }
    ...
    $('#' + music_id).click(function () {
      ...
      if (hostUrl.match(/music\.163\./)) {
        if (hostUrl.match(/^https?:\/\/music\.163\.com\/#\/(?:song|dj)\?id/)) {
          window.open(
            'http://www.eggvod.cn/newmusic/?url=' +
              encodeURIComponent(hostUrl.replace('/#', ''))
          );
        } else if (
          hostUrl.match(/^https?:\/\/y\.music\.163\.com\/m\/(?:song|dj)\?id/)
        ) {
          window.open(
            'http://www.eggvod.cn/newmusic/?id=' +
              encodeURIComponent(
                'https://music.163.com/song?id=' + getQueryString('id')
              )
          );
        } else {
          ...
          if (songId[1]) {
            window.open(
              'http://www.eggvod.cn/newmusic/?url=' +
                encodeURIComponent('https://music.163.com/song?id=' + songId[1])
            );
          } else {
            return false;
          }
        ...
        try {
          ...
          alert(
            'QQ音乐请在单曲播放页面点击VIP图标解析，在列表和单曲详情页面中点击VIP图标无效'
          );
        }

        if ($('#sim_song_info').children('a').attr('href')) {
          var qqSong = $('#sim_song_info')
            .children('a')
            .attr('href')
            .match(/song\/(\S*).html/);
        } else {
          qqSong = $('.player_music__info')
            .children('a')
            .attr('href')
            .match(/songDetail\/(\S*)\?/);
        }
        ...
        window.open(
          'http://www.eggvod.cn/newmusic/?id=' +
            qqSong[1] +
            '&type=qq&playUrl=' +
            encodeURIComponent(document.querySelector('audio').src)
        );
