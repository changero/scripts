...
(function () {
  ...
  obj.fetchVideoInfoHomePage = function (callback) {
    var instanceForSystem = obj.require(
    ...
    freeList.forEach(function (a, index) {
      obj.video_page.quality.push({
        ...
        url:
          ...
          encodeURIComponent(
            obj.video_page.adToken ? obj.video_page.adToken : ''
          ),
        type: 'hls',
      });
    });
  ...
  obj.freeList = function (e) {
    ...
    var t = [480, 360],
      ...
      i = +a[1] * +a[2];
    return i
      ? (i > 409920 && t.unshift(720), i > 921600 && t.unshift(1080), t)
      : t;
  };
  ...
  obj.dPlayerStart = function () {
    var dPlayerNode,
      videoNode = document.getElementById('video-wrap');
    if (videoNode) {
    ...
    try {
      ...
      location.pathname == '/mbox/streampage' &&
        $(dPlayerNode).css('height', '480px');
  ...
  obj.playSetting = function () {
    ...
    localStorage.getItem('dplayer-autoposition') == 'true' &&
      ($('.dplayer-toggle-setting-input-autoposition').get(0).checked = true);
    localStorage.getItem('dplayer-autoplaynext') ||
      localStorage.setItem('dplayer-autoplaynext', true);
    localStorage.getItem('dplayer-autoplaynext') == 'true' &&
      ($('.dplayer-toggle-setting-input-autoplaynext').get(0).checked = true);

    $('.dplayer-setting-autoposition').on('click', function () {
      var autoposition = !$('.dplayer-toggle-setting-input-autoposition').is(
        ':checked'
      );
      $('.dplayer-toggle-setting-input-autoposition').get(0).checked =
        autoposition;
      localStorage.setItem('dplayer-autoposition', autoposition);
    });
    $('.dplayer-setting-autoplaynext').on('click', function () {
      var autoplaynext = !$('.dplayer-toggle-setting-input-autoplaynext').is(
        ':checked'
      );
      $('.dplayer-toggle-setting-input-autoplaynext').get(0).checked =
        autoplaynext;
  ...
  obj.autoPlayNext = function () {
    ...
    if (nextSibling.length) {
      ...
      location.href =
        'https://pan.baidu.com/play/video#/video?path=' +
  ...
  obj.getJquery = function () {
    return (
      unsafeWindow.$ ||
      unsafeWindow.jQuery ||
      obj.require('base:widget/libs/jquerypacket.js')
    );
  };
